// 대한민국 행정구역: 시/도 → 시/군/구 (읍/면/동은 자유 입력)
// 2024 기준(강원·전북특별자치도, 대구 군위군 편입 반영)
...
// "서울특별시 강남구 역삼동" → { sido, sigungu, dong }
export function parseRegion(value?: string | null): {
  sido: string;
  sigungu: string;
  dong: string;
} {
  ...
  if (!sido) return { sido: "", sigungu: "", dong: value.trim() };
  const rest = parts.slice(1);
  const sigungu = REGIONS[sido].includes(rest[0]) ? rest[0] : "";
  const dong = (sigungu ? rest.slice(1) : rest).join(" ");
