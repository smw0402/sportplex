...
export default async function HomePage({
  ...
  const [news, popularPosts, openRecruits, coaches, layout] = await Promise.all([
  ...
  const blocks: Record<string, React.ReactNode> = {
      ...
      <section key="news">
        ...
        {news.length === 0 ? (
          ...
          <div className="space-y-4">
            {hero && (
              <Link href={`/news/${hero.id}`} className="card-link block overflow-hidden">
                {hero.imageUrl ? (
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              {rest.map((n) => (
                <Link key={n.id} href={`/news/${n.id}`} className="card-link block p-4">
    ...
    recruits: (
      <section key="recruits" className="card p-5">
        <div className="mb-3 flex items-center justify-between">
    ...
    coaches: (
      <section key="coaches" className="card p-5">
  ...
  return (
    <div className="space-y-6 md:space-y-8">
      ...
      {/* 관리자 레이아웃 설정 순서대로 블록 렌더 */}
      {layout.filter((b) => b.visible).map((b) => blocks[b.key]).filter(Boolean)}
      ...
      {!user && (
        <section className="card bg-brand-50 p-5 text-center">
          ...
          <Link href="/signup" className="btn-primary mx-auto mt-3 inline-flex px-8">
            무료로 시작하기
          </Link>
        </section>
      )}
